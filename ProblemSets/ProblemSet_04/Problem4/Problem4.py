...
#Taken from problem 2 - modified to include tau constraint
def calc_chi_sqrd(f, m, y_true, N_inv):
    ...
    chi_sqrd = res.T@N_inv@res + (np.subtract(true_tau, m[3])/true_tau_err)**2
...
# Obtain the initial parameters (I chose the ones from problem 1 - with covariance from p3)
m_init = [69, 0.022, 0.12, 0.06, 2.1e-9, 0.95]
p3_chain = np.loadtxt("../Problem3/planck_chain.txt", delimiter=" ")[100:, :]

true_tau = 0.054
true_tau_err = 0.0074

# The covariance through importance sampling of the problem 3 chain
m_covariance = np.cov(p3_chain[:, 1:], aweights=np.exp(-0.5*(np.subtract(true_tau, p3_chain[:, 3])/true_tau_err)**2), rowvar=False)

# Collect and save the chain - Uncomment the next two lines to run the chain
...
for i in range(0, 3):
    for j in range(0, 2):
        ...
        ax[i][j].loglog(ft)
        index += 1
plt.savefig("FourierTransform_Problem4")
plt.cla()
plt.clf()
plt.close()

## Comparing to importance sampling of problem 3 ##

p3_m_mean_importance = np.average(p3_chain[:, 1:], weights=np.exp(-0.5*(np.subtract(true_tau, p3_chain[:, 3])/true_tau_err)**2), axis=0)
p3_m_std_importance = np.sqrt(np.diag(m_covariance))


print("-- Results through importance sampling --")

print("The parameters are", p3_m_mean_importance, "//", np.shape(data_c_mean))
print(" ")
print("The standard deviations are", p3_m_std_importance)
print(" ")
